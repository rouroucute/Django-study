...
class MyAnsiable2():
    def __init__(self,
        ...
        # 动态添加组
        self.inv_obj.add_group(group_name)

        # 动态添加组内成员
        self.inv_obj.add_host(host_ip, group_name)

        # 动态给组添加变量
        group_obj = self.inv_obj.groups[group_name]
        if varsdic:
            shellkey, shellval = varsdic.item()
            group_obj.set_variable(shellkey, shellval)
        else:
            group_obj.set_variable("", "")

        # 变量管理器
        self.variable_manager = VariableManager(self.loader, self.inv_obj)

    def run(self, hosts='localhost', gether_facts="no", module=' ', args=' '):
        play_source = dict(
            name="Ad-hoc",
            hosts=hosts,
...
def ansible_run(group_name, host_ip, varsdic, modelname, shellname):

    ansible2 = MyAnsiable2(inventory='/auto_cmdb/octopus/inventory/hosts',
                           connection='smart',
                           remote_user="stus",
                           host_ip=host_ip,
                           varsdic=varsdic,
    ...
    ansible2.run(hosts=group_name, module=modelname, args=shellname)
